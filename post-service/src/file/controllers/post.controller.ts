...
export class PostController {
  ...
  @UseGuards(AuthGuard('jwt'))
  @Get()
  async postById(@Req() req: Request, @Query('post_id', ParseIntPipe) post_id?: number) {
    const acc_id = typeof(req['user'].id) === 'string' ? parseInt(req['user'].id) : req['user'].id;
    // const acc_id = 1;
  ...
  @UseGuards(AuthGuard('jwt'))
  @Post()
  async createPost(@Req() req: Request, @Body() postDto: PostReqDto): Promise<any> {
    const acc_id = typeof(req['user'].id) === 'string' ? parseInt(req['user'].id) : req['user'].id;
    // const acc_id = 1;
  ...
  @UseGuards(AuthGuard('jwt'))
  @Patch()
  async updatePost(@Req() req: Request, @Body() postDto: PostReqDto, @Query('post_id', ParseIntPipe) post_id?: number): Promise<any> {
    const acc_id = typeof(req['user'].id) === 'string' ? parseInt(req['user'].id) : req['user'].id;
    // const acc_id = 1;
    if(postDto != null && postDto.min_gpa == null) {
      postDto.min_gpa = 0;
  ...
  @UseGuards(AuthGuard('jwt'))
  @Delete()
  async deletePost(@Req() req: Request, @Query('post_id', ParseIntPipe) post_id?: number): Promise<any> {
    const acc_id = typeof(req['user'].id) === 'string' ? parseInt(req['user'].id) : req['user'].id;
    // const acc_id = 1;
