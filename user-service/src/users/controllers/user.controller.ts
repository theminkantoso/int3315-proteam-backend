...
import { Body, Controller, Get, NotFoundException, Param, ParseIntPipe, Patch, Post, Query, Req, Request, UseGuards } from '@nestjs/common';
import {
...
//   @ApiResponse({ status: 200, description: 'Forbidden.' })
  ...
  @Query('id', ParseIntPipe) id?: number): Promise<any> {
    ...
    if(acc_id == 10000) {
      // admin acc
      throw new NotFoundException("Unable to retrieve this user");
    }
      ...
      if (profile) {
        if (status == 1) {
          ...
          if (profile.role != 2) {
            ...
            // tick xanh
            let return_field = instanceToPlain(profile);
            return_field['friend'] = relation.account_id == acc_id ? 2 : 3;
            return return_field;
          }
        } 
      }
